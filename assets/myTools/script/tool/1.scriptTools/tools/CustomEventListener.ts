...
export class CustomEventListener extends Component 
{
    ...
    public static on(eventName:string,cb:Function,target:any)//监听
    {
    ...
    public static off(eventName:string,cb:Function,target?:any)//取消监听
    {
    ...
    public static dispatchEvent(eventName:string,...args:any)//消息派发
    {
        const list =this.handle[eventName];
        if (!list||list.length<=0)
        {
