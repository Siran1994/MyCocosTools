import { _decorator, Component, Node, Prefab, Vec3, instantiate, director, RigidBody, Enum, Camera, Quat } from 'cc';
import { FollowTarget } from '../common/FollowTarget';
...
export class Cannon extends Component
{
    ...
    @property( { type: Prefab, displayName: '发射物', group: { name: '炮弹', id: '2' } } )
     ...
     * @param targetPos 
     */
    public fire ( followCamera: Node, targetPos?: Vec3 )
    {
        // 瞄准
        targetPos && this.aim( targetPos );
        // 发射
        this.shoot( this.velocity, followCamera );
    }
     ...
     * @param velocity 初始速度
     */
    protected shoot ( velocity: number, followCamera: Node )
    {
        // 获取炮弹
        const bulletNode = this.generateBullet(),
            bulletRigidBody = bulletNode.getComponent( RigidBody );
        var follow = followCamera.getComponent( FollowTarget );
        follow.target = bulletNode;
        follow.node.eulerAngles = new Vec3( -30, -15, 0 );
        follow.node.worldPosition = new Vec3( -5, 8, -50 );
        follow.offset = new Vec3( -2, 6, 8 );
        follow.enabled = true;
