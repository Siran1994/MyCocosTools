import { _decorator, Component, EventHandler, EventTouch, Input, Node, UITransform } from 'cc';
...
export class LongTouch extends Component
{
    ...
    private _onTouchStart ( event: EventTouch )
    {
        ...
        if ( !this.enableMultiTouching )
        {
            ...
            if ( this.node.getComponent( UITransform ).getBoundingBoxToWorld().contains( event.getUILocation() ) )
    ...
    private publishOneTouch ()
    {
        if ( this.node.active == false )
            this.unschedule( this._touchCounterCallback );
        if ( !this._isTouching && this.node.active == false )
        {
            return;
        }
        this._touchCounter++;
