import { _decorator, Component, CameraComponent, screen, input, Input, geometry, Vec2, macro, sys, view, Prefab, PhysicsSystem, ToggleComponent, Node, resources, instantiate, LabelComponent } from 'cc';
    ...
    async start() {
        ...
        resources.load("Prefab/Toggle", Prefab, (err, prefab) => {
            for (var i in eleColor) {
                ...
                toggle.name = i;
                toggle.getChildByName("Label").getComponent(LabelComponent).string=eleColor[i].name;
